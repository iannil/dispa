...
pub enum AuthMode { ApiKey, Bearer }
...
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct RsaJwk { pub kid: Option<String>, pub n: String, pub e: String }
...
impl SecurityManager {
    ...
    pub fn max_body_bytes(&self) -> Option<u64> { self.cfg.ddos.as_ref().and_then(|d| d.max_body_bytes) }

    pub async fn check_request(&self, req: &Request<Body>, client_ip: Option<IpAddr>) -> Option<Response<Body>> {
        if !self.cfg.enabled { return None; }

        // DDoS: basic header limits
        if let Some(dd) = &self.cfg.ddos {
            if let Some(max) = dd.max_headers { if req.headers().len() > max { metrics::counter!("dispa_security_denied_total", &[("kind", String::from("headers_len"))]).increment(1); return Some(resp_431()); } }
            if let Some(maxb) = dd.max_header_bytes { if headers_size(req) > maxb { metrics::counter!("dispa_security_denied_total", &[("kind", String::from("headers_size"))]).increment(1); return Some(resp_431()); } }
            if let Some(max_body) = dd.max_body_bytes {
                if let Some(len) = req.headers().get(hyper::header::CONTENT_LENGTH).and_then(|v| v.to_str().ok()).and_then(|s| s.parse::<u64>().ok()) {
                    if len > max_body { metrics::counter!("dispa_security_denied_total", &[("kind", String::from("body_too_large"))]).increment(1); return Some(resp_413()); }
                } else if dd.require_content_length.unwrap_or(false) {
                    metrics::counter!("dispa_security_denied_total", &[("kind", String::from("length_required"))]).increment(1);
        ...
        // Rate limit by IP
        if let Some(rl) = &self.cfg.rate_limit { if rl.enabled {
            let key = client_ip.map(|ip| ip.to_string()).unwrap_or_else(|| "unknown".to_string());
            let now = Instant::now();
            let mut map = self.rate_map.lock().await;
            let mut st = *map.get(&key).unwrap_or(&RateState { tokens: rl.burst, last: now });
            ...
            drop(map);
            if !allow { metrics::counter!("dispa_security_denied_total", &[("kind", String::from("rate_limit"))]).increment(1); return Some(resp_429()); }
        }}
        ...
        if let Some(ac) = &self.cfg.access_control {
            let ip = client_ip.or_else(|| {
                if ac.trust_proxy_headers {
                    ...
                    req.headers().get("x-forwarded-for").and_then(|h| h.to_str().ok()).and_then(|s| s.split(',').next()).and_then(|s| s.trim().parse::<IpAddr>().ok())
                } else { None }
            });
            if let Some(ip) = ip {
                if let Some(denied) = &ac.denied_ips {
                    if denied.iter().any(|p| ip_match(p, &ip)) { metrics::counter!("dispa_security_denied_total", &[("kind", String::from("denied_ip"))]).increment(1); return Some(resp_403()); }
                }
                if let Some(allowed) = &ac.allowed_ips {
                    if !allowed.iter().any(|p| ip_match(p, &ip)) { metrics::counter!("dispa_security_denied_total", &[("kind", String::from("not_allowed_ip"))]).increment(1); return Some(resp_403()); }
                }
            }
        }

        // JWT (Bearer) validation if configured
        if let Some(jwt) = &self.cfg.jwt { if jwt.enabled {
            let name = self.cfg.auth.as_ref().and_then(|a| a.header_name.clone()).unwrap_or_else(|| "authorization".into());
            if let Some(val) = req.headers().get(&name).and_then(|h| h.to_str().ok()) {
                if let Some(token) = val.strip_prefix("Bearer ").or_else(|| val.strip_prefix("bearer ")) {
                    if !self.verify_jwt(token, jwt).await {
                        metrics::counter!("dispa_security_denied_total", &[("kind", String::from("jwt_invalid"))]).increment(1);
                        return Some(resp_401("Invalid token"));
                    }
                } else {
                    metrics::counter!("dispa_security_denied_total", &[("kind", String::from("auth_missing"))]).increment(1);
        ...
        if let Some(auth) = &self.cfg.auth { if auth.enabled {
            match auth.mode {
                AuthMode::ApiKey => {
                    let name = auth.header_name.clone().unwrap_or_else(|| "x-api-key".into());
                    let v = req.headers().get(name).and_then(|h| h.to_str().ok());
                    if v.is_none() || !auth.keys.iter().any(|k| v.unwrap() == k) { metrics::counter!("dispa_security_denied_total", &[("kind", String::from("auth_apikey"))]).increment(1); return Some(resp_401("API key required")); }
                }
                AuthMode::Bearer => {
                    // If JWT already configured, this static list can be used as allowlist fallback (optional)
                    let name = auth.header_name.clone().unwrap_or_else(|| "authorization".into());
                    let v = req.headers().get(name).and_then(|h| h.to_str().ok());
                    if let Some(val) = v {
                        let token = val.strip_prefix("Bearer ").or_else(|| val.strip_prefix("bearer ")).unwrap_or("");
                        if !auth.keys.iter().any(|k| token == k) { metrics::counter!("dispa_security_denied_total", &[("kind", String::from("auth_bearer"))]).increment(1); return Some(resp_401("Invalid token")); }
                    } else { metrics::counter!("dispa_security_denied_total", &[("kind", String::from("auth_missing"))]).increment(1); return Some(resp_401("Unauthorized")); }
                }
            }
        }}
    ...
    async fn verify_jwt(&self, token: &str, cfg: &JwtConfig) -> bool {
        match cfg.algorithm.to_ascii_uppercase().as_str() {
            "HS256" => {
                ...
                // Compute HMAC
                let secret = match cfg.secret.as_ref() { Some(s)=>s.as_bytes(), None=>return false };
                let signing_input = format!("{}.{}", h_b64, p_b64);
                let mac = hmac_sha256(secret, signing_input.as_bytes());
                if mac != sig { return false; }
                if !validate_claims(&payload, cfg).await { return false; }
                // Cache expiration
            ...
            "RS256" => {
                #[cfg(feature = "jwt-rs256")]
                {
                    ...
                    // Resolve RSA key
                    let key = match self.resolve_rs256_key(cfg, &kid).await { Some(k)=>k, None=>return false };
                    let signing_input = format!("{}.{}", h_b64, p_b64);
                    if !rsa_verify_sha256(&key.n, &key.e, signing_input.as_bytes(), &sig) { return false; }
                    if !validate_claims(&payload, cfg).await { return false; }
                    cache_token_exp(&self.jwt_cache, token, &payload, cfg).await;
                    true
                }
...
fn decode_jwt_parts(token: &str) -> Option<(String, String, String, serde_json::Value, serde_json::Value, Vec<u8>)> {
    let mut parts = token.split('.');
    let (h_b64, p_b64, s_b64) = (parts.next()?.to_string(), parts.next()?.to_string(), parts.next()?.to_string());
...
async fn validate_claims(payload_json: &serde_json::Value, cfg: &JwtConfig) -> bool {
    ...
    if let Some(exp) = payload_json.get("exp").and_then(|v| v.as_i64()) { if now > exp + leeway { return false; } }
    if let Some(nbf) = payload_json.get("nbf").and_then(|v| v.as_i64()) { if now + leeway < nbf { return false; } }
    if let Some(iat) = payload_json.get("iat").and_then(|v| v.as_i64()) { if iat - leeway > now { return false; } }
    if let Some(iss) = cfg.issuer.as_ref() { if payload_json.get("iss").and_then(|v| v.as_str()) != Some(iss.as_str()) { return false; } }
    if let Some(aud) = cfg.audience.as_ref() { match payload_json.get("aud") { Some(serde_json::Value::String(s)) if s==aud => {}, _ => return false } }
    true
}

async fn cache_token_exp(cache: &std::sync::Arc<tokio::sync::Mutex<HashMap<String, std::time::Instant>>>, token: &str, payload_json: &serde_json::Value, cfg: &JwtConfig) {
    if cfg.cache_enabled.unwrap_or(true) {
        let now = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_secs() as i64;
        if let Some(exp) = payload_json.get("exp").and_then(|v| v.as_i64()) {
            let ttl = if exp > now { (exp - now) as u64 } else { 0 };
            cache.lock().await.insert(token.to_string(), std::time::Instant::now() + std::time::Duration::from_secs(ttl));
        }
    }
}
...
fn rsa_verify_sha256(n_b64url: &str, e_b64url: &str, msg: &[u8], sig: &[u8]) -> bool {
    let n = match b64url_decode(n_b64url) { Some(v)=>v, None=>return false };
    let e = match b64url_decode(e_b64url) { Some(v)=>v, None=>return false };
    let pk = ring::signature::RsaPublicKeyComponents{ n: &n, e: &e };
    pk.verify(&ring::signature::RSA_PKCS1_2048_8192_SHA256, msg, sig).is_ok()
}
...
impl SecurityManager {
    async fn fetch_jwks(&self, url: &str, ttl_secs: u64) -> HashMap<String, RsaJwk> {
        #[cfg(feature = "jwt-rs256-net")]
        {
            ...
            if let Ok(resp) = reqwest::get(url).await { if let Ok(text) = resp.text().await {
                if let Ok(json) = serde_json::from_str::<serde_json::Value>(&text) {
                    if let Some(keys) = json.get("keys").and_then(|k| k.as_array()) {
                        for k in keys {
                            if k.get("kty").and_then(|x| x.as_str()) == Some("RSA") && k.get("alg").and_then(|x| x.as_str()) == Some("RS256") {
                                if let (Some(n), Some(e)) = (k.get("n").and_then(|x| x.as_str()), k.get("e").and_then(|x| x.as_str())) {
                                    let kid = k.get("kid").and_then(|x| x.as_str()).map(|s| s.to_string());
                                    let jwk = RsaJwk{ kid: kid.clone(), n: n.to_string(), e: e.to_string() };
                                    if let Some(kid) = kid { map.insert(kid, jwk); }
                                }
                            }
                        }
                    }
...
async fn resolve_rs256_key(&self, cfg: &JwtConfig, kid: &Option<String>) -> Option<RsaJwk> {
    ...
    if let Some(keys) = &cfg.rs256_keys {
        if let Some(k) = kid {
            if let Some(j) = keys.iter().find(|j| j.kid.as_ref() == Some(k)) { return Some(j.clone()); }
        } else if let Some(j) = keys.first() { return Some(j.clone()); }
    }
    ...
    #[cfg(feature = "jwt-rs256-net")]
    {
        if let Some(url) = &cfg.jwks_url { let ttl = cfg.jwks_cache_secs.unwrap_or(600);
            ...
            for (kid_s, jwk) in map.into_iter() { cache.insert(kid_s.clone(), JwkCacheEntry{ key: jwk, exp }); }
            if let Some(k) = kid { return cache.get(k).map(|e| e.key.clone()); }
...
fn hmac_sha256(key: &[u8], msg: &[u8]) -> Vec<u8> {
    ...
    let mut k = if key.len() > BLOCK { let mut hasher = Sha256::new(); hasher.update(key); hasher.finalize().to_vec() } else { key.to_vec() };
    if k.len() < BLOCK { k.resize(BLOCK, 0); }
    let mut ipad = vec![0x36u8; BLOCK];
    let mut opad = vec![0x5cu8; BLOCK];
    for i in 0..BLOCK { ipad[i] ^= k[i]; opad[i] ^= k[i]; }
    let mut ih = Sha256::new(); ih.update(&ipad); ih.update(msg); let inner = ih.finalize();
    let mut oh = Sha256::new(); oh.update(&opad); oh.update(&inner); oh.finalize().to_vec()
}
...
fn base64_decode(s: &str) -> Option<Vec<u8>> {
    ...
    let mut i = 0;
    while i < bytes.len() {
        let a = val(bytes[i])?; let b = val(bytes[i+1])?; let c = val(bytes[i+2])?; let d = val(bytes[i+3])?; i+=4;
...
fn resp_429() -> Response<Body> { Response::builder().status(StatusCode::TOO_MANY_REQUESTS).header("Retry-After", "1").body(Body::from("Rate limited")).unwrap() }
fn resp_431() -> Response<Body> { Response::builder().status(StatusCode::REQUEST_HEADER_FIELDS_TOO_LARGE).body(Body::from("Headers too large")).unwrap() }
fn resp_413() -> Response<Body> { Response::builder().status(StatusCode::PAYLOAD_TOO_LARGE).body(Body::from("Payload too large")).unwrap() }
fn resp_411() -> Response<Body> { Response::builder().status(StatusCode::LENGTH_REQUIRED).body(Body::from("Content-Length required")).unwrap() }
fn resp_403() -> Response<Body> { Response::builder().status(StatusCode::FORBIDDEN).body(Body::from("Forbidden")).unwrap() }
fn resp_401(msg: &str) -> Response<Body> { Response::builder().status(StatusCode::UNAUTHORIZED).header("WWW-Authenticate", "Bearer").body(Body::from(msg.to_string())).unwrap() }
...
mod tests {
    ...
    async fn test_ddos_header_limits() {
        let cfg = SecurityConfig{
            ...
            auth: None,
            rate_limit: None,
            ddos: Some(DdosConfig{ max_headers: Some(1), max_header_bytes: None, max_body_bytes: None, require_content_length: None }),
            jwt: None,
        };
    ...
    async fn test_jwt_hs256_validation() {
        ...
        let signing_input = format!("{}.{}", b64url_enc(header), b64url_enc(payload.as_bytes()));
        ...
        let cfg = SecurityConfig {
            ...
            rate_limit: None,
            ddos: None,
            jwt: Some(JwtConfig { enabled: true, algorithm: "HS256".into(), secret: Some("secret".into()), leeway_secs: Some(5), issuer: Some("me".into()), audience: Some("you".into()), cache_enabled: Some(true), rs256_keys: None, jwks_url: None, jwks_cache_secs: None })
        };
        let mgr = SecurityManager::new(cfg);
    ...
    async fn test_jwt_claims_issuer_audience_leeway() {
        ...
        let payload = format!("{{\"iss\":\"me\",\"aud\":\"you\",\"exp\":{},\"iat\":{}}}", now + 60, iat);
        fn b64url_enc(data: &[u8]) -> String { base64_encode(data).replace('+', "-").replace('/', "_").trim_end_matches('=').to_string() }
        let signing_input = format!("{}.{}", b64url_enc(header), b64url_enc(payload.as_bytes()));
        let sig = hmac_sha256(b"secret", signing_input.as_bytes());
        let token = format!("{}.{}", signing_input, b64url_enc(&sig));

        let cfg = SecurityConfig {
            enabled: true, access_control: None, auth: None, rate_limit: None, ddos: None,
            jwt: Some(JwtConfig { enabled: true, algorithm: "HS256".into(), secret: Some("secret".into()), leeway_secs: Some(5), issuer: Some("me".into()), audience: Some("you".into()), cache_enabled: Some(true), rs256_keys: None, jwks_url: None, jwks_cache_secs: None })
        };
        let mgr = SecurityManager::new(cfg);
        let req = Request::builder().uri("/").header("authorization", format!("Bearer {}", token)).body(Body::empty()).unwrap();
        ...
        let signing_input2 = format!("{}.{}", b64url_enc(header), b64url_enc(payload2.as_bytes()));
