...
impl TrafficLogger {
    ...
    pub async fn log_request(
        ...
        match cfg.log_type {
            ...
            LoggingType::Both => {
                ...
                metrics::histogram!("dispa_log_write_duration_ms", &file_labels)
                    .record(file_elapsed);
                match file_res {
                    Ok(_) => metrics::counter!("dispa_log_writes_total", &file_labels).increment(1),
                    Err(e) => {
                        metrics::counter!("dispa_log_write_errors_total", &file_labels)
                            .increment(1);
    ...
    async fn restart_cleanup_task(&self, retention_days: u32) {
        ...
        *self.cleanup_handle.write().unwrap() = Some(handle);
        info!(
            "Started traffic log cleanup task (retention_days = {})",
            retention_days
        );
    }
}
...
mod tests {
    ...
    async fn test_concurrent_logging() {
        let _ = tokio::time::timeout(std::time::Duration::from_secs(15), async {
            ...
            for i in 0..10 {
                ...
                let handle = tokio::spawn(async move {
                    let request_id = Uuid::new_v4();
                    let client_addr =
                        SocketAddr::new(IpAddr::V4(Ipv4Addr::new(10, 0, 0, i + 1)), 8080);
                    logger_clone
                        .log_request(
                            request_id,
                            client_addr,
            ...
            if let Some(ref pool) = pool_opt {
                let count: i64 =
                    sqlx::query_scalar("SELECT COUNT(*) FROM traffic_logs WHERE host = ?")
        ...
        .await
        .expect("test_concurrent_logging timed out");
    }
    ...
    async fn test_logging_concurrent_requests() {
        ...
        .await
        .expect("test_logging_concurrent_requests timed out");
    }
