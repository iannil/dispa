...
use dispa::config::{
    Config, DomainConfig, FileConfig, HealthCheckConfig, HttpClientConfig, LoadBalancingConfig,
...
fn create_end_to_end_test_config(target_urls: Vec<String>) -> Config {
    let targets: Vec<Target> = target_urls
        ...
        .map(|(i, url)| Target {
            ...
            url: url.clone(),
            address: url.clone(),         // Keep full URL for validation
            weight: Some((i + 1) as f64), // Different weights for testing
            timeout: Some(30),
        })
        .collect();
    ...
    Config {
        ...
        logging: LoggingConfig {
            ...
            file: Some(FileConfig {
                ...
                rotation: false,
                max_file_size: Some(10 * 1024 * 1024), // 10MB in bytes
            }),
            retention_days: None,
        },
...
async fn test_end_to_end_proxy_with_load_balancing() {
    ...
    .await
    .expect("test_end_to_end_proxy_with_load_balancing timed out");
}
...
async fn test_domain_matching_integration() {
    ...
    .await
    .expect("test_domain_matching_integration timed out");
}
...
async fn test_health_checking_integration() {
    ...
    .await
    .expect("test_health_checking_integration timed out");
}
...
async fn test_different_load_balancing_algorithms() {
    tokio::time::timeout(std::time::Duration::from_secs(20), async {
        ...
        for algorithm in algorithms {
            ...
            config.http_client = Some(dispa::config::HttpClientConfig {
                pool_max_idle_per_host: Some(8),
                pool_idle_timeout: Some(30),
                pool_idle_timeout_secs: Some(30),
                connect_timeout: Some(2),
    ...
    .await
    .expect("test_different_load_balancing_algorithms timed out");
}
...
async fn test_logging_integration() {
    ...
    .await
    .expect("test_logging_integration timed out");
}
...
async fn test_monitoring_integration() {
    ...
    .await
    .expect("test_monitoring_integration timed out");
}
...
async fn test_configuration_validation_integration() {
    tokio::time::timeout(std::time::Duration::from_secs(20), async {
        let test_cases = vec![
            ...
            // Empty domains (actually valid - will handle all traffic with warning)
            (true, vec!["http://backend1.test".to_string()], vec![]),
        ];
        ...
        for (should_be_valid, target_urls, domains) in test_cases {
            ...
            if should_be_valid {
                ...
                assert!(
                    ...
                    "Configuration should be invalid but validation passed. Config: {:?}",
                    config
                );
            }
        }
    ...
    .await
    .expect("test_configuration_validation_integration timed out");
}
...
async fn test_concurrent_component_creation() {
    ...
    .await
    .expect("test_concurrent_component_creation timed out");
}
...
async fn test_component_interaction_patterns() {
    ...
    .await
    .expect("test_component_interaction_patterns timed out");
}
